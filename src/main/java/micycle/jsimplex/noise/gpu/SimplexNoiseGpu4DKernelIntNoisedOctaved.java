...
class SimplexNoiseGpu4DKernelIntNoisedOctaved extends Kernel {
	...
	public void run() {
		int i = getGlobalId();
		int x = i % argsInt[0];
		int y = i % (argsInt[0] * argsInt[1] * argsInt[2]) % (argsInt[0] * argsInt[1]) / argsInt[1];
		...
		for (int j = 0; j < argsInt[4]; j++) {
			...
			accumulator += noise(argsFloat[0] * power + newFrequency * x, argsFloat[1] * power + newFrequency * y,
					argsFloat[2] * power + newFrequency * z, argsFloat[3] * power + newFrequency * w, j);
		}
	...
	public void setParameters(float x, float y, float z, float w, int width, int height, int depth, int depth4,
