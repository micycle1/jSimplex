...
public class SimplexNoiseGpu2D {
	...
	public static synchronized float[] calculate(float x, float y, int width, int height, float frequency) {
		simplexKernel.setParameters(x, y, width, height, frequency);
		simplexKernel.execute(width * height);
		return simplexKernel.getResult();
	}

	public static synchronized float[] calculateFast(float x, float y, int width, int height, float frequency) {
	...
	public static synchronized float[] calculateOctaved(float x, float y, int width, int height, int depth, int depth4,
			float frequency, float[] weight) {
		simplexKernelOctaved.setParameters(x, y, width, height, frequency, weight);
		simplexKernelOctaved.execute(width * height);
		return simplexKernelOctaved.getResult();
	}

	public static synchronized float[] calculateFastOctaved(float x, float y, int width, int height, int depth,
			int depth4, float frequency, float[] weight) {
		fastSimplexKernelOctaved.setParameters(x, y, width, height, frequency, weight);
